...
def harrison_ford
  ...
  Movie.joins(:actors).joins(:castings).where(actors: {name: 'Harrison Ford'}).where('castings.ord > 1').select(:id, :title).distinct
end
...
def biggest_cast
  ...
  Movie.joins(:castings).group(:id).order('COUNT(*) DESC').limit(3).select(:id, :title)
end
